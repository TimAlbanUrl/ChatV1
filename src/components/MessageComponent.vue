...
/*<div v-if="message.super" class="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 p-1 rounded-lg shadow-2xl w-auto h-auto"> */
</script>

<template>
    <div class="ml-1 p-4 pb-2 flex rounded-lg bg-gray-800 w-[31.5rem] shadow-2xl">
        <img :src="avatarUrl" alt="users_avatar" class="rounded-full h-14 w-14">
        <div class="pl-2">
            <div class="flex">
                <p class="font-bold m-1">{{ message.author.username }}</p>
            ...
            <article class="w-96 h-min-24 text-wrap-b">
                ...
                <button class="stroke-2 text-gray-600 px-1 mt-1" @click="emit('like', message.id)">
        ...
        <button v-if="message.author.id == user?.id" class="place-self-start stroke-2 text-gray-600" @click="emit('delete', message.id)">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
